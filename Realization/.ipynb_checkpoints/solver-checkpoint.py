...
def cls_estimator_cv(Data:np.ndarray, labels:list, model='RF') -> np.float32:
    scoring = ['f1']
    if model == 'RF':
    ...
    if model == 'LogRegression':
        ...
        clsf = linear_model.LogisticRegression(**res.best_params_,random_state=42)
    if model == 'AdaBoost':
        ...
        clsf = ensemble.AdaBoostClassifier(**res.best_params_, random_state=42)
...
def CV_boosting_clf(Data:np.ndarray, labels:list, model='LightGBM') -> np.float32:
    scoring = ['f1']
    if model == 'LightGBM':
